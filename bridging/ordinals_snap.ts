...
import { RemoteSigner } from "../bob/sdk/src/ordinals";
...
import { inscribeData } from "../bob/sdk/src/ordinals";
import { createTextInscription } from "../bob/sdk/src/ordinals/commit";
import { MAX_CHUNK_SIZE } from "../bob/sdk/src/ordinals/commit";
import { chunkContent } from "../bob/sdk/src/ordinals/commit";

export class StaticSigner implements RemoteSigner {
...
export async function inscribeText(secret: string, toAddress: string, amount: number, text: string, fee: number): Promise<Transaction> {
    const signer = new StaticSigner(secret);
    return await inscribeData(signer, toAddress, amount, createTextInscription(text), fee);
}